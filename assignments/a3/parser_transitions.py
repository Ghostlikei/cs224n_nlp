...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            t_from = self.stack[-1]
            t_to = self.stack[-2]
            self.dependencies.append((t_from, t_to))
            self.stack.remove(t_to)
        elif transition == "RA":
            rt_from = self.stack[-2]
            rt_to = self.stack[-1]
            self.dependencies.append((rt_from, rt_to))
            self.stack.remove(rt_to)
    ...
    def finished(self):
        """
        @return boolean: true if buffer is empty and size of stack is one
        """
