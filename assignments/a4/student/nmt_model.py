...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Initialize encoder (Bidirectional LSTM)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded)  # Assuming you have a model_embeddings layer

        # Step 2: Apply the post_embed_cnn layer
        X = X.permute(1, 2, 0)  # Permute dimensions for CNN (b, e, src_len)
        X = self.post_embed_cnn(X)  # Apply your convolutional layers
        X = X.permute(2, 0, 1)  # Revert X back to the original shape (src_len, b, e)

        # Step 3: Compute enc_hiddens, last_hidden, last_cell by applying the encoder
        ...
        # Pad the packed sequence to obtain enc_hiddens
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True)
        # print("Enc hidden before perm:", enc_hiddens.size())

        # Permute enc_hiddens to the desired shape (b, src_len, h*2)
        # enc_hiddens = enc_hiddens.permute(0, 2, 1)
        # print("Enc hidden after perm:", enc_hiddens.size())

        # Step 4: Compute dec_init_state
        # Concatenate the forwards and backwards tensors
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (b, 2*h)

        # Apply the h_projection layer to obtain init_decoder_hidden
        init_decoder_hidden = self.h_projection(last_hidden)  # Assuming you have an h_projection layer

        # Concatenate the forwards and backwards tensors for last_cell
        last_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)  # (b, 2*h)

        # Apply the c_projection layer to obtain init_decoder_cell
        init_decoder_cell = self.c_projection(last_cell)  # Assuming you have a c_projection layer

        # Return enc_hiddens, and a tuple of init_decoder_hidden and init_decoder_cell
        return enc_hiddens, (init_decoder_hidden.unsqueeze(0)[0], init_decoder_cell.unsqueeze(0)[0])
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Step 2: Construct tensor Y of target sentences with shape (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)

        # Step 3: Iterate over the time dimension of Y
        for Y_t in torch.split(Y, 1):
            Y_t = Y_t.squeeze()  # Squeeze to shape (b, e)
            ...
            step_output = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            dec_state, o_t = step_output[0], step_output[1]
        ...
        # Step 4: Stack combined_outputs to get the final output
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        # Step 1: Apply the decoder LSTM cell to Ybar_t and dec_state
        ...
        # Step 3: Apply softmax to e_t to get alpha_t
        alpha_t = F.softmax(e_t, dim=1)

        # Step 4: Compute the attention output a_t
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # Step 5: Concatenate dec_hidden and a_t to compute U_t
        U_t = torch.cat([dec_hidden, a_t], dim=1)

        # Step 6: Apply the combined output projection layer to U_t
        V_t = self.combined_output_projection(U_t)

        # Step 7: Compute tensor O_t by applying Tanh and dropout
